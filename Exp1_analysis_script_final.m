...
for p = 1:length(participants)
    for c = 1:length(conditions)
        for s = 1:length(sessions)
            ...
            elseif size(unique(session_data.block),1) == 4 && ~sum(contains(unique(session_data.block), 'pre1'))
                blocks = {'pre2', 'pst1', 'pst2', 'pst3'};
            ...
            if sum(contains(blocks, 'pre1'))
                ...
                sp_pre_avg = log_means.pre2.protocol_0;
            ...
            for protocol = paired_protocols
                ...
                if sum(contains(blocks, 'pre1'))
                    ...
                    ratio_pre_avg.(sprintf('protocol_%d', protocol)) = log_means.pre2.(sprintf('protocol_%d', protocol));
